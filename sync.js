...
async function grab(u){try{return await(await fetch(u,{headers:{"User-Agent":"Mozilla/5.0"},signal:AbortSignal.timeout(15000)})).text();}catch{return "";}}
async function httpcode(u){try{return (await fetch(u,{headers:{"User-Agent":"Mozilla/5.0"},signal:AbortSignal.timeout(12000)})).status;}catch{return 0;}}
async function seoReal(u){const h=await grab(u);if(!h)return 0;const t=re=>re.test(h);const title=(h.match(/<title[^>]*>([^<]*)<\/title>/i)||[])[1]||"";const dtag=(h.match(/<meta[^>]*name=["']description["'][^>]*>/i)||[])[0]||"";const desc=(dtag.match(/content=["']([^"']*)["']/i)||[])[1]||"";const h1=(h.match(/<h1[\s>]/gi)||[]).length;const rt=await httpcode(u.replace(/\/?$/,"/")+"robots.txt"),sm=await httpcode(u.replace(/\/?$/,"/")+"sitemap.xml");const c=[title.length>=10&&title.length<=65,desc.length>=50&&desc.length<=170,t(/rel=["']canonical["']/i),t(/og:title/i),t(/og:image/i),t(/twitter:card/i),t(/name=["']robots["']/i),t(/ld\+json/i),h1===1,t(/viewport/i),t(/<html[^>]*lang=/i),t(/G-[A-Z0-9]{10}/),t(/rel=["'][^"']*icon/i),rt===200,sm===200,t(/preconnect/i)];return Math.round(c.filter(Boolean).length/c.length*100);}
const BLOGS=[
...
{name:"VisionScore AI",domain:"visionscoreai.agenciafadamadrinha.com",url:"https://visionscoreai.agenciafadamadrinha.com/",sc:"https://visionscoreai.agenciafadamadrinha.com/",ga4:"544659124",mid:"G-92WW3LMQ3D",nicho:"IA Preditiva de Tráfego",fixedIdx:1},
{name:"Agência Fada Madrinha",domain:"agenciafadamadrinha.com",url:"https://agenciafadamadrinha.com/",sc:"https://agenciafadamadrinha.com/",ga4:"530381421",mid:"G-EEQYMQ0QC9",nicho:"Agência / Serviços",fixedIdx:1},
{name:"Universidade Fada Madrinha",domain:"universidadefadamadrinha.com",url:"https://universidadefadamadrinha.com/",sc:"https://universidadefadamadrinha.com/",ga4:"",mid:"G-DEQ6Q4WVWV",nicho:"Curso / Educação",fixedIdx:1},
{name:"Comunidade Fada Madrinha",domain:"comunidadefadamadrinha.com",url:"https://comunidadefadamadrinha.com/",sc:"https://comunidadefadamadrinha.com/",ga4:"",mid:"G-DEQ6Q4WVWV",nicho:"Comunidade",fixedIdx:1},
];
(async()=>{
...
const seo_score=await seoReal(b.url);
